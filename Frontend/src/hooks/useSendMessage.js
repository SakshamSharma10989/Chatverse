  ...
  const sendMessage = async (content) => {
    ...
    try {
      ...
      if (typeof content === "string") {
        console.log("Sending text message to:", `/api/message/send/${selectedConversation._id}`);
        ...
        console.log("Sending file to:", `/api/message/upload/${selectedConversation._id}`, content.name);
        const formData = new FormData();
        formData.append("media", content);
        const res = await fetch(`/api/message/upload/${selectedConversation._id}`, {
          method: "POST",
          body: formData,
        });
      ...
      setMessages([...messages, data]); // Ensure this updates the store
